...
class CPUMetricsCallback(Callback):
    ...
    def on_train_epoch_end(self, trainer: "Trainer", pl_module: "LightningModule") -> None:
        ...
        #     max_memory = self._torch_runner.max_memory_allocated(self.root_gpu(trainer)) / 2**20
        # else:
        ...
        if hasattr(self._torch_runner, "max_memory_allocated"):
            ...
            max_memory = float("nan")
        # max_memory: float = self._torch_runner.max_memory_allocated(self.root_device(trainer)) / 2**20
        epoch_time: float = time.time() - self.start_time
